...
export class ZohoService {
  ...
  async exchangeCodeForToken(code: string) {
    ...
    const clientSecret = this.configService.getOrThrow<string>('ZOHO_CLIENT_SECRET');
    const redirectUri = this.configService.getOrThrow<string>('ZOHO_REDIRECT_URI');
  ...
  async upsertContact(user: any) {
    ...
    const createRes = await fetch('https://www.zohoapis.in/crm/v2/Contacts', {
      method: 'POST',
      headers: {
  ...
  async initializeToken() {
    const refreshToken = this.configService.getOrThrow<string>('ZOHO_REFRESH_TOKEN');
  ...
  async getValidAccessToken(): Promise<string> {
    ...
    if (!tokenDoc) throw new Error('Zoho token not initialized');
  ...
  async getItemImage(zohoItemId: string): Promise<Buffer | null> {
    try {
      const accessToken = await this.getValidAccessToken();

      const response = await fetch(
        `https://www.zohoapis.in/inventory/v1/items/${zohoItemId}/image`,
        { headers: { Authorization: `Zoho-oauthtoken ${accessToken}` } },
      );

      if (!response.ok) {
        console.warn(`Zoho image not found for item ${zohoItemId}: ${response.status}`);
        return null;
      }
      ...
      console.error('Failed to fetch Zoho item image:', error);
      return null;
    }
  }

  // ── NEW METHOD ────────────────────────────────────────────
  // Fetches full item details from Zoho Inventory by item ID.
  // Used to verify or re-fetch item data when needed.
  async getItemById(zohoItemId: string): Promise<any> {
    const accessToken = await this.getValidAccessToken();

    const response = await fetch(
      `https://www.zohoapis.in/inventory/v1/items/${zohoItemId}`,
      { headers: { Authorization: `Zoho-oauthtoken ${accessToken}` } },
    );
